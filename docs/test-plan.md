...
| TC2 | Add new branch worktree with default path | 1. Using TC1 fixture, run `wt add feature/foo`. | Worktree directory created under base with branch checked out; branch exists in repo; directory name sanitised (`feature-foo`). |
| TC3 | List worktrees | After TC2, execute `wt ls --porcelain`. | Output contains the added worktree path. |
| TC4 | Move worktree to new relative path | 1. Using TC2 fixture, run `wt move feature-foo moved`. | Worktree relocated to `<base>/moved`; `git worktree list` reflects new path. |
